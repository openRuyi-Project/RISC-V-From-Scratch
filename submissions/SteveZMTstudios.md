...
```bash
# 解压 + 创建磁盘镜像
mkdir -p rootfs-riscv64-lfs-SteveZMTstudios
tar -xf rootfs-riscv64-lfs-SteveZMTstudios.tar.zst -C rootfs-riscv64-lfs-SteveZMTstudios
...
## 已知问题 / TODO
- 虚拟机内需要手动初始化网络，因此在发布映像一段时间后启动，可能会看到如下错误：
  
  ```
  [FAILED] Failed to start Update PKI/TLS certificate store.
  See 'systemctl status update-pki.service' for details.
  ```

  这是因为 `make-ca` 需要连接到 Internet 来下载证书颁发机构列表。可以按照如下方式配置网络：
  ...
  此时再次运行 `systemctl restart update-pki` ，`make-ca` 就会开始同步证书或发现今日无事可做。
